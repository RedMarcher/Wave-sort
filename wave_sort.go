...
func (s seq) blockSwap(m, r, p int) {
	...
	l := p - m + 1
	bs += l // count block swapped elements
	tmp := s[m]
	init := m
	j := m
	nm := p - ll + 1
...
// r is the index of the right bound of the left block and the first element of the right block.
// p is the index of the last element of the right block.
func (s seq) blockSwap_e(m, r, p int) {
	ll := r - m
	if ll == 0 {
		return
	}
...
func (s seq) upwave_e(start, end int) {
	length := end - start + 1
	if length == 1 {
		return
	}
...
func (s seq) downwave_e(start, sortedStart, end int) {
